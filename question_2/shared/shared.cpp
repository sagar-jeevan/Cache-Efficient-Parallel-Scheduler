...
void MMBase(int x_i, int x_j, int y_i, int y_j, int z_i, int z_j, Sync *sync, int id)
{
    ...
    if(sync && get_counter(sync) == 4) {
        ...
        while(currentSync != NULL) {
            if(currentSync && currentSync->flag == 0 && get_counter(currentSync) == 4) {
                ...
                MMSecondHalf(newSync->x_i, newSync->x_j,
                    ...
                    break;
            } else if(currentSync && currentSync->flag == 1 && get_counter(currentSync) == 4) {
...
void pushBackTask(Task* toPush, int threadID)
{
    ...
    work_load[threadID] = work_load[threadID] + pow(toPush->functionPara.n, 3);
...
Task *getBackTask(int threadID)
{
    ...
    pthread_mutex_lock(&workLock);
    work_load[threadID] = work_load[threadID] - pow(newTask->functionPara.n, 3);
    pthread_mutex_unlock(&workLock);
    pthread_mutex_unlock(&dequeLock[threadID]);
...
int main(int argc, char *argv[])
{
	...
	float efficieny = 1979.904 / (__cilkrts_get_nworkers() * runTime);
	cout << "Efficieny:" << efficieny << endl;
	return 0;
}

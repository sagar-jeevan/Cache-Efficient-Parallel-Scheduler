...
int checkLoad(int rand1, int rand2){
    ...
    if(workLoad[rand1] < workLoad[rand2])
	out = rand1;
    else
        out = rand2;
...
void MMFirstHalf(int x_i, int x_j, int y_i, int y_j, int z_i, int z_j, int n, Sync *sync, int id)
{
    ...
    while(rand2 == rand1)
	rand2 = randomNumber(id);
    pushBackTask(initTask(x_i, x_j, y_i, y_j, z_i, z_j, n/2, newSync, 0), checkLoad(rand1, rand2));
    rand1 = randomNumber(id);
    rand2 = randomNumber(id);
    while(rand2 == rand1)
        rand2 = randomNumber(id);
    pushBackTask(initTask(x_i, x_j, y_i, y_j + n/2, z_i, z_j + n/2, n/2, newSync, 0), checkLoad(rand1, rand2));
    rand1 = randomNumber(id);
    rand2 = randomNumber(id);
    while(rand2 == rand1)
        rand2 = randomNumber(id);
    pushBackTask(initTask(x_i + n/2, x_j, y_i, y_j, z_i + n/2, z_j, n/2, newSync, 0), checkLoad(rand1, rand2));
    rand1 = randomNumber(id);
    rand2 = randomNumber(id);
    while(rand2 == rand1)
        rand2 = randomNumber(id);
    pushBackTask(initTask(x_i + n/2, x_j, y_i, y_j + n/2, z_i + n/2, z_j + n/2, n/2, newSync, 0), checkLoad(rand1, rand2));
}
...
void MMSecondHalf(int x_i, int x_j, int y_i, int y_j, int z_i, int z_j, int n, Sync *sync, int id)
{
    ...
    while(rand2 == rand1)
        rand2 = randomNumber(id);
    pushBackTask(initTask(x_i, x_j + n/2, y_i + n/2, y_j, z_i, z_j, n/2, sync, 1), checkLoad(rand1, rand2));
    rand1 = randomNumber(id);
    rand2 = randomNumber(id);
    while(rand2 == rand1)
        rand2 = randomNumber(id);
    pushBackTask(initTask(x_i, x_j + n/2, y_i + n/2, y_j + n/2, z_i, z_j + n/2, n/2, sync, 1),checkLoad(rand1,rand2));
    rand1 = randomNumber(id);
    rand2 = randomNumber(id);
    while(rand2 == rand1)
        rand2 = randomNumber(id);
    pushBackTask(initTask(x_i + n/2, x_j + n/2, y_i + n/2, y_j, z_i + n/2, z_j, n/2, sync, 1), checkLoad(rand1,rand2));
    rand1 = randomNumber(id);
    rand2 = randomNumber(id);
    while(rand2 == rand1)
        rand2 = randomNumber(id);
    pushBackTask(initTask(x_i + n/2, x_j + n/2, y_i + n/2, y_j + n/2, z_i + n/2, z_j + n/2, n/2, sync, 1), checkLoad(rand1, rand2));
}
...
void runThread(int threadID)
{
    while(!workDone) {
        if (!myQueue[threadID].empty()) {
            Task *newTask = getBackTask(threadID);
            if (newTask)
...
int main(int argc, char *argv[])
{
	...
	cout << "Number of Workers\n" << __cilkrts_get_nworkers() << endl;
        cout << "Efficieny:" << 18.4863 / (__cilkrts_get_nworkers() * runTime) << endl;	
